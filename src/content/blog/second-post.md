...
All of these interact in nonlinear ways. More airflow means a hotter fire, but it also means more oxygen to the meat and faster moisture loss. A water pan stabilizes temperature but also steals heat that could be going to the cooking grate. A bigger fire is more forgiving of small adjustments but harder to control precisely.

I want to build a simulation where I can tweak each parameter and see the effect instantly. Think of it as a control theory problem with a really tasty solution. The model will use finite difference methods to simulate the heat flow through the system, treating the firebox, cooking chamber, water basin, and grate as coupled thermal nodes with conductive, convective, and radiative heat transfer between them.
...
Will I actually use any of the physics I simulate? Probably not, but it'll be interesting to see what I come up with. Maybe a deeper understanding of the system will lead to more intuition when I'm actually out there tending the fire. I'll post updates here as I make progress.
